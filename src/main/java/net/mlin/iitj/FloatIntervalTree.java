...
/**
 * Data structure for storing float [begin, end) intervals and answering requests for all items
...
public class FloatIntervalTree implements java.io.Serializable {
    public static class Builder {
        ...
        public void add(float beg, float end, Object tag) {
            if (beg > end) {
        ...
        public void add(float beg, float end) {
            add(beg, end, null);
        }

        public FloatIntervalTree build() {
            FloatIntervalTree ans = new FloatIntervalTree(this);
            reset();
            return ans;
        }
        ...
        private void grow() {
            ...
            float[] tmp = new float[(int) capacity];
            for (int i = 0; i < n; i++) {
                tmp[i] = begs[i];
            }
            begs = tmp;
            tmp = new float[(int) capacity];
            for (int i = 0; i < n; i++) {
                tmp[i] = ends[i];
            }
            ends = tmp;
    ...
    public FloatIntervalTree(Builder builder) {
        ...
        begs = new float[builder.n];
        ends = new float[builder.n];
        maxEnds = new float[builder.n];
    ...
    public static class QueryResult {
        ...
        public QueryResult(float beg, float end, Object obj) {
            this.beg = beg;
            this.end = end;
            this.obj = obj;
        }
        ...
        public String toString() {
            // for debugging
            String ans =
                    "[" + (new Float(beg)).toString() + "," + (new Float(end)).toString() + ")";
            if (obj != null) {
                ans += "=" + obj.toString();
            }
            return ans;
    ...
    public void queryOverlap(
            Float queryBeg, Float queryEnd, Function<QueryResult, Boolean> callback) {
        for (int which_i = 0; which_i < indexNodes.length - 1; which_i++) {
            int i = indexNodes[which_i];
            if (begs[i] >= queryEnd) {
                break;
    ...
    public QueryResult queryAnyExact(Float queryBeg, Float queryEnd) {
        QueryResult[] ans = {null};
        queryExact(
                queryBeg,
                queryEnd,
    ...
    public void queryAll(Function<QueryResult, Boolean> callback) {
        queryOverlap(Float.MIN_VALUE, Float.MAX_VALUE, callback);
    }
