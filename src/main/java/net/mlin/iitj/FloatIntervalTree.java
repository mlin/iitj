...
public class FloatIntervalTree implements java.io.Serializable {
    ...
    private FloatIntervalTree(Builder builder) {
        ...
        maxEnds = new float[nIndexNodes][0];
        for (int which_i = 0; which_i < indexNodes.length; ++which_i) {
            ...
            final float[] maxEnd = new float[1 + (n_i - 1) / 4];
            if (n_i >= 8) {
    ...
    private void recurseMaxEnds(float[] ans, int ofs, int node, int lvl) {
    ...
    public void validate() {
        ...
        for (int which_i = 0; which_i < indexNodes.length; ++which_i) {
            ...
            final int n_i =
                    (which_i + 1 < indexNodes.length ? indexNodes[which_i + 1] : begs.length) - i;
            final float[] maxEnd = maxEnds[which_i];
    ...
    private class QueryContext {
        ...
        public float[] maxEnd = null;
        // offset of leftmost leaf in main item array
        public int ofs = -1;

        public QueryContext(float beg, float end, IntPredicate callback) {
            this.beg = beg;
            this.end = end;
