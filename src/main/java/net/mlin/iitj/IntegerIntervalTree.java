...
/**
 * Data structure for storing int [begin, end) intervals and answering requests for all items
...
public class IntegerIntervalTree implements java.io.Serializable {
    public static class Builder {
        ...
        public void add(int beg, int end, Object tag) {
            if (beg > end) {
        ...
        public IntegerIntervalTree build() {
            IntegerIntervalTree ans = new IntegerIntervalTree(this);
            reset();
            return ans;
        }
    ...
    public IntegerIntervalTree(Builder builder) {
        ...
        begs = new int[builder.n];
        ends = new int[builder.n];
        maxEnds = new int[builder.n];
    ...
    public static class QueryResult {
        ...
        public QueryResult(int beg, int end, Object obj) {
            this.beg = beg;
            this.end = end;
            this.obj = obj;
        }
        ...
        public String toString() {
            // for debugging
            String ans =
                    "[" + (new Integer(beg)).toString() + "," + (new Integer(end)).toString() + ")";
            if (obj != null) {
                ans += "=" + obj.toString();
            }
            return ans;
    ...
    public void queryOverlap(
            Integer queryBeg, Integer queryEnd, Function<QueryResult, Boolean> callback) {
        for (int which_i = 0; which_i < indexNodes.length - 1; which_i++) {
            int i = indexNodes[which_i];
            if (begs[i] >= queryEnd) {
                break;
    ...
    public java.util.List<QueryResult> queryOverlap(Integer queryBeg, Integer queryEnd) {
    ...
    public QueryResult queryAnyOverlap(Integer queryBeg, Integer queryEnd) {
    ...
    public boolean queryOverlapExists(Integer queryBeg, Integer queryEnd) {
    ...
    public QueryResult queryAnyExact(Integer queryBeg, Integer queryEnd) {
        QueryResult[] ans = {null};
        queryExact(
                queryBeg,
                queryEnd,
    ...
    public boolean queryExactExists(Integer queryBeg, Integer queryEnd) {
        return queryAnyExact(queryBeg, queryEnd) != null;
    }

    public void queryAll(Function<QueryResult, Boolean> callback) {
        queryOverlap(Integer.MIN_VALUE, Integer.MAX_VALUE, callback);
    }
    ...
    private static Integer max(Integer lhs, Integer rhs) {
